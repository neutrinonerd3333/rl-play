...
class QLearner:
    ...
    def select_action(self, state: str, verbose: bool = False) -> int:
        ...
        return self.q_approximator.best_action(state, verbose=verbose) if greedy else random.randrange(self.action_n)
    ...
    def update_q(self, old_state, new_state, action, reward, terminal):
        ...
        """
        self.q_approximator.update(old_state, new_state, action, reward, terminal, self.current_gamma, learning_rate=self.current_learning_rate)
    ...
    def decay(self, i_episode):
        ...
        self.current_gamma = min(self.gamma_final, self.gamma + i_episode * self.gamma_anneal_rate)
